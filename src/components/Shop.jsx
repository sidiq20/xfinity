            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {category.items.map((item, itemIndex) => (
                <div key={itemIndex} className="bg-white rounded-lg shadow-lg p-6 hover:shadow-xl transition-shadow">
                  ...
                  {item.speed && (
                    <p className="text-2xl text-xfinity-blue font-bold mb-2">
                      {item.speed}
                    </p>
                  )}
                  {item.data && (
                  ...
                  <button className="w-full bg-xfinity-blue text-white rounded-full py-2 px-4 hover:bg-blue-700 transition-colors">
                    Choose Plan
                  </button>
                </div>
              ))}
